...
export default function AllPhotosScreen() {
  ...
  const { user } = useAuth();
  const { allPhotos, loadAllPhotos, removePhotos, movePhotos } = useGallery();
  ...
  }, [selectedIds, allPhotos, removePhotos, exitSelectionMode, showAlert]);

  const handleMoveSelected = useCallback(
    async (targetAlbum: Album) => {
      const toMove = allPhotos.filter((p) => selectedIds.has(p.id));
    ...
    [allPhotos, selectedIds, movePhotos, exitSelectionMode, showAlert],
  );
